...
int main(int argc,char **argv)
{
	...
	z=0;
	vel_msg.linear.y=0;
	out_msg1.linear.z=30;
	out_msg1.angular.x=0;
	out_msg1.angular.y=0;
	out_msg1.angular.z=0;
...
void controller_callback(const sensor_msgs::Joy::ConstPtr& msg)
{
    ...
    if (msg->buttons[4]==1)
    	out_msg1.linear.z = out_msg1.linear.z + 5 ;   // yeh wla servo ka angle hain water flow

     if (msg->buttons[5]==1)                         //  yeh wla bhi servo ka hain water flow
    	out_msg1.linear.z = out_msg1.linear.z - 5 ;
 	
    if (msg->axes[7]==1 )
    	out_msg1.angular.x = out_msg1.angular.x + 5;   // servo for nozzle

    if (msg->axes[7]==-1 )
        out_msg1.angular.x = out_msg1.angular.x - 5;  //servo for nozzle

    if (msg->axes[6]==1 )
    	out_msg1.angular.y = 1-out_msg1.angular.y;    //yeh wala rotating platform ka hei

    if (msg->axes[6]==-1 )
     	out_msg1.angular.y = -1 - out_msg1.angular.y; //yeh wala bhi rotating platform

    
    if (msg->axes[2] == -1 )
     	out_msg1.angular.z = -1 - out_msg1.angular.z;   // yeh wla fog lamp ka hei
